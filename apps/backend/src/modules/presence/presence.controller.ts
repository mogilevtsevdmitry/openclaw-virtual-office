...
export class PresenceController {
  ...
  /**
   * PUT /api/v1/presence/internal/:agentId
   * Internal endpoint — called by pipeline-worker (no JWT required)
   * Updates presence state for a pipeline-driven state change
   */
  @Public()
  @Put('internal/:agentId')
  @HttpCode(HttpStatus.OK)
  async changeActivityInternal(
    @Param('agentId') agentId: string,
    @Body() dto: ChangeActivityDto,
  ) {
    return this.changeActivityUseCase.execute(agentId, dto);
